...
// Notice! 保持merge一般的语义， a.Merge(b)后，a变了，a中优先起作用的将是b里的
func (rs *Ruleset) Merge(m *Ruleset) {
	for k, v := range m.uncountables {
		rs.uncountables[k] = v
	}
	rs.plurals = append(m.plurals, rs.plurals...)
	rs.singulars = append(m.singulars, rs.singulars...)
	rs.humans = append(m.humans, rs.humans...)
	rs.acronyms = append(m.acronyms, rs.acronyms...)
}
